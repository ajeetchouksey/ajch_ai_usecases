...
1. Read 2-3 existing case files in `content/usecases/cases/` closest to the brief's vertical or pattern, to match voice and depth (e.g. `loan-application-underwriting.json` for banking/HITL shape).
...
```
{
  ...
  "relatedInterviewQs": ["existing q-ids from ajch_platform's interview question bank, if a genuine match exists — omit rather than invent"],
